...
!      real, external::rand
      ...
      integer i,j,k,ift,ia,ja,ii,ita,jseed
        ...
        do k=lau_low_limit,lau_up_limit
          do j=1,mp                           ! scan to find launching cell
            do i=1,np
              ...
              do ift=1,nfuel
                if (((temps(ift,i,j,k).GE.temphot)
     +            .AND.(rhos(ift,i,j,k).GE.fden_limit))
     +            .AND.(xvb(i,j,k,3).GT.0)) then
!
...
!ccc
                      ...
                      new_fb(nbmax3,10)=maxval(temps(:,i,j,k)) 
                      if (ishape.EQ.8) then
...
! effective firebrand
            if(kb.EQ.1) then
              fuel_lnd_hgt=zs_real+0.5*maxval(actualfueldepth(:,ib,jb,kb))
            else
              fuel_lnd_hgt=zs_real+0.5*maxval(actualfueldepth(:,ib,jb,kb))
     +                         +maxval(actualfueldepth(:,ib,jb,kb-1))
            endif !kb.eq.1    


            if(sum(rhof(:,ib,jb,kb)).GT.fden_limit
...
!            print*,"travel dist:",travel_dist,temps(1,ib,jb,kb)
     ...
     +        .AND.maxval(temps(:,ib,jb,kb)).LT.temphot) then
              imm_new=imm_new+1
              ibrand(nb,2)=5
...
!              print*,"landed on fuel cell!!!",nb,mpi_rank 
     ...
     +        .AND.maxval(temps(:,ib,jb,kb)).GE.temphot) then ! ibrand(nb,2)=5 
...
!              print*,"landed on fire",nb,mpi_rank 
          ...
          if ((bz.LE.zs_real .OR. bz.LT.0.0).and. idn.eq.0 ) then
            brand(nb,3)=zs(ib,jb)
            imm_new=imm_new+1
            if ((maxval(temps(:,ib,jb,kb)).LT.temphot)
     +        .AND.(sum(rhof(:,ib,jb,kb)).GT.fden_limit)
     +        ) then
              ibrand(nb,2)=5
c             print*,"landed on fuel",nb,mpi_rank
              nfb_imm(ib,jb,kb)=nfb_imm(ib,jb,kb)+1
              idn=1
            elseif (maxval(temps(:,ib,jb,kb)).GE.temphot) then
              ibrand(nb,2)=6
        ...
        if (iland_fuel.NE.1 .and. idn.eq.0)then
          if (kb.EQ.1) then
            ...
            if (bz.LE.zs_real .OR. bz.LT.0.0) then
              ...
              if ((maxval(temps(:,ib,jb,kb)).LT.temphot) 
     +          .AND.
     +          (sum(rhos(:,ib,jb,kb)).GT.fden_limit) 
...
!             print*,'nfb_imm',i,j,k,nfb_imm(i,j,k),temps(1,i,j,k)
              if((minval(temps(:,i,j,k)).LT.1000 !targettemp(1) !targettemp=1000
     +          .and.sum(rhof(:,i,j,k)).GT.fden_limit) )then

                temp_cri=((nfb_imm(i,j,k)+0.1)/nb_per_cell-2.0)*350+300
                if(temp_cri.GT.1000) temp_cri=1000
                do ift=1,nfuel
                  if(temps(ift,i,j,k).LT.temp_cri) then
                    temps(ift,i,j,k)=temps(ift,i,j,k)+
