...
c
     ...
     .ifuelinra,fuelinranumber,ifuel,nfuel,ivegread,
     .rhomicrovalue, cpwood,waterProductToWoodMassRatio,
     ...
     .ifbrand,                                 !KOO
     .ifp,
     .idirt !AJJ 01/08/20 List of void legacy variables that might still
            ! be on gridlists
...
!mmc 6/28/06 added to prevent mixups with inonlocal and irhovapor, and nv in gridlist
      ...
      if (nfuel==0) then 
        nfuel = 1 ! in case nfuel is unspecified (AJJ 8/9/19)
      endif
!
! FIXME 
      iheatsource=0  
...
!
      ...
      call MPI_Bcast(relativehumidity,1,mpi_real,0,mpi_comm_world,ierr)
      ...
      call MPI_Bcast(nfuel,1,mpi_integer,0,mpi_comm_world,ierr)
      call MPI_Bcast(ivegread,1,mpi_integer,0,mpi_comm_world,ierr)
      call MPI_Bcast(cpwood,1,mpi_real,0,mpi_comm_world,ierr)
      call MPI_Bcast(waterProductToWoodMassRatio,1,mpi_real,0,mpi_comm_world,ierr)
...
c walltimer !KOO
      ...
      if (irhovapor.eq.1.and.nv.eq.8) nv=nv+1  ! in case somebody forgot to
                        !increase nv to 9 in a rhovapor run
      call con
      call rinitmsg
      call definearray
      call rinit(tcv)
...
c        endif
        if (iwallclock.EQ.1)wtime2=MPI_Wtime()
         ...
         if (ilspgf.ge.1.and.it/frqlspgf*frqlspgf.eq.it) call largeScalePGF(it)
            ...
            if (frqfilstr>0) call filstr() ! do filter every 10 time step
