...
c234567
     ...
     .   d11b,d22b,d33b,d12b,d13b,d23b,1-ih,np+ih,1-ih,mp+ih,l,nv)     
         ...
         do k=1,l
            do j=1,mp
               do i=1,np
                  pr(i,j,k)=(xvb(i,j,k,4)*rg_air/prrcp_air)**(cp_air/cv_air)
                  tempg(i,j,k)=xvb(i,j,k,4)/xvb(i,j,k,nv)
     +                 *(pr(i,j,k)*1.e-5)**(rg_air/cp_air)
               enddo
            enddo
         enddo
...
c      av=2.*(rhof(i,j,k)*0.004+rhowater(i,j,k)*.0001)/ss    !rrl
     ...
     &      *(pre(i,j,k)*1.0e-5)**(rg_air/cp_air)
...
c      endif
c      endif
      capqterm=energy*(1.e5/pr(i,j,k))**(rg_air/cp_air)/cp_air            !rrl
