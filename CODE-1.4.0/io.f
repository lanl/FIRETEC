...
!----------------------------------------------------------
     ...
     +                         status='unknown')
      ...
      if(irod.eq.1)then
        ...
        if(inonlocal.eq.1) call writeio(xvb(1-ih,1-ih,1,8),21,1-ih,np+ih,1-ih,mp+ih,l,0)
        if(irhovapor.eq.1) call writeio(xvb(1-ih,1-ih,1,8),21,1-ih,np+ih,1-ih,mp+ih,l,0) ! field 11
        if (irad.GE.1) call writeio(firad,21,1-ih,np+ih,1-ih,mp+ih,l,0)       ! field 23
        ...
        if (iradeastflux.eq.1) call writeio(eastFlux,21,1-ih,np+ih,1-ih,mp+ih,l,0)
        if (inonlocal.eq.1) call writeio(fg,21,1-ih,np+ih,1-ih,mp+ih,l,0) !jjc
        do ift=1,nfuel 
          call writeio(rhof(ift,:,:,:),21,1-ih,np+ih,1-ih,mp+ih,l,0) ! field 7
          call writeio(rhofinitial(ift,:,:,:),21,1-ih,np+ih,1-ih,mp+ih,l,0)     ! field 8
          call writeio(rhowater(ift,:,:,:),21,1-ih,np+ih,1-ih,mp+ih,l,0)        ! field 15
          call writeio(actualfueldepth(ift,:,:,:),21,1-ih,np+ih,1-ih,mp+ih,l,0) ! field 17
          call writeio(sizescale(ift,:,:,:),21,1-ih,np+ih,1-ih,mp+ih,l,0) ! field 17
          call writeio(temps(ift,:,:,:),21,1-ih,np+ih,1-ih,mp+ih,l,1)              ! field 9
          call writeio(sies(ift,:,:,:),21,1-ih,np+ih,1-ih,mp+ih,l,0)            ! field 12
          call writeio(psiwmax(ift,:,:,:),21,1-ih,np+ih,1-ih,mp+ih,l,0)         ! field 16
          call writeio(convht(ift,:,:,:),21,1-ih,np+ih,1-ih,mp+ih,l,0)  ! field 22
          if (irad.GE.1) call writeio(frhosiesrad(ift,:,:,:),21,1-ih,np+ih,1-ih,mp+ih,l,0)
        enddo
...
!----------------------------------------------------------------------------
     ...
     +  open(41,file=fname,form='unformatted',status='old')
      ...
      if(irod.eq.1) then
        ...
        if(inonlocal.eq.1) call readio(xvb(1-ih,1-ih,1,8),41,l,1-ih,np+ih,1-ih,mp+ih)
        if(irhovapor.eq.1) call readio(xvb(1-ih,1-ih,1,8),41,l,1-ih,np+ih,1-ih,mp+ih)
        if (irad.GE.1) call readio(firad,41,l,1-ih,np+ih,1-ih,mp+ih) !field 23
        if (ilspgf.ge.1) call readio(sinthetaf,41,l,1-ih,np+ih,1-ih,mp+ih) 
        do ift=1,nfuel
          if (irst.eq.1) then
            call readio(rhof(ift,:,:,:),41,l,1-ih,np+ih,1-ih,mp+ih)
            call readio(rhofinitial(ift,:,:,:),41,l,1-ih,np+ih,1-ih,mp+ih)
            call readio(rhowater(ift,:,:,:),41,l,1-ih,np+ih,1-ih,mp+ih) !mv fp
            call readio(actualfueldepth(ift,:,:,:),41,l,1-ih,np+ih,1-ih,mp+ih) !mv fp
            call readio(sizescale(ift,:,:,:),41,l,1-ih,np+ih,1-ih,mp+ih) !mv fp
            ...
            call    readio(unused,41,l,1-ih,np+ih,1-ih,mp+ih)
            call    readio(unused,41,l,1-ih,np+ih,1-ih,mp+ih)
          endif !irst.eq.2.or.0
          call readio(temps(ift,:,:,:),41,l,1-ih,np+ih,1-ih,mp+ih)
          call readio(sies(ift,:,:,:),41,l,1-ih,np+ih,1-ih,mp+ih)
          call readio(psiwmax(ift,:,:,:),41,l,1-ih,np+ih,1-ih,mp+ih)
          call readio(convht(ift,:,:,:),41,l,1-ih,np+ih,1-ih,mp+ih)
          if (irad.GE.1) call readio(frhosiesrad(ift,:,:,:),41,l,1-ih,np+ih,1-ih,mp+ih)
        enddo
      endif
...
c
      if (mpi_rank.eq.0)
     +  allocate(tmparray(il:iu,jl:ju,nzdim,nproc),outdata(n,m,nzdim))
...
c
      ...
      ! iflag = 1 deals with temps for fire position
        if(iflag.eq.1) then
          do ia=2,n-1
            do ja=2,m-1
              if (outdata(ia,ja,1).gt.600) front=real(i)
            enddo
          enddo
