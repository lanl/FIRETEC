...
! FP 09/2019 : added multiple fuel in radiation in a crude mode: here we
! assumes that all the different fuel types are spread in the first zone
! in the first grid cell
     ...
     .     zcoordsf(:,:,:) ! for mpi_fluxes
      ...
      integer::iradMultiFuelSimple=1 ! for a first simple implementation
     .         !of multiple fuel in which fuel types are merged in the
     .         ! first zone
      ...
      real :: t4barsolid, t4bar !FP does not have to be an array
      real :: sourceSolSum !FP sum over ift
...
!!!--------------------------subroutine frad_init  -----------------------------------!!!
      ...
      integer :: i,j,k,ift,kk,cnt,cnt_rate,cnt_max
      ...
      allocate(sourcesol(nfuel,1-ih:np+ih,1-ih:mp+ih,lhighlim)); sourcesol=0.0
      ...
      if(irad.eq.1)then 
       ...
       allocate(papvift(nfuel,1-ih:np+ih,1-ih:mp+ih,llowlim:lhighlim)); papvift = 0.0
      ...
      if(irad.eq.2)then 
        if (iradMultiFuelSimple.eq.1) then
       ...
       ! for k=3 to l+lphase, arrayAZ(:,:,k)=array(:,:,k-1) : solid+gas
        else ! version with different zones
          lphase=nfuel
          write(6,*) 'full version of multifuel for radiation not 
     &                implemented yet'
          STOP
        endif 
        allocate(papvift(nfuel,np,mp,lhighlim)); papvift = 0.0  !projected area per unit volume of the solid
       ...
       Emin=1000.0
        papvthresh = 2.0*min_rhof/nfuel/(ss*rhomicrovalue)
...
! FP09/2019 ...UNDER PROGRESS!
       do k=1,lphase
          do j=1,mp
             do i=1,np
                minfueldepth=0.1*(zcartedgeAZ1(i,j,lphase+1)-zcartedgeAZ1(i,j,0))
              zcartedgeAZ1(i,j,k)= zcartedgeAZ1(i,j,0)+max(maxval(actualfueldepth(:,i,j,1)),minfueldepth) 
                minfueldepth=0.1*(zcartedgeAZ2(i,j,lphase+1)-zcartedgeAZ2(i,j,0))
              zcartedgeAZ2(i,j,k)= zcartedgeAZ2(i,j,0)+max(maxval(actualfueldepth(:,i,j,1)),minfueldepth) 
                minfueldepth=0.1*(zcartedgeAZ3(i,j,lphase+1)-zcartedgeAZ3(i,j,0))
              zcartedgeAZ3(i,j,k)= zcartedgeAZ3(i,j,0)+max(maxval(actualfueldepth(:,i,j,1)),minfueldepth) 
                minfueldepth=0.1*(zcartedgeAZ4(i,j,lphase+1)-zcartedgeAZ4(i,j,0))
              zcartedgeAZ4(i,j,k)= zcartedgeAZ4(i,j,0)+max(maxval(actualfueldepth(:,i,j,1)),minfueldepth) 
             enddo !i
          enddo !j
       enddo !k
! correction for actualfueldepth
! check that zcartedgeAZ(i,j,1) are relevant due to actualfueldeth:
! (adding 1 to nzonebelowcell1 in order to create artificial zones with
! same papv for array size homogeneity)
       do k=1,lphase
          do j=1,mp
             do i=1,np
...
!!!----------------------- subroutine papv_defs -----------------------------------!!!
      ...
      integer :: i,j,k,ift,kreal,ia,ja !,msgcnt
      ...
      do k=1,l
         do j=1,mp
          do i=1,np
              ...
              pr=(xvb(i,j,k,4)*rg_over_prrcp_gas)**(cp_over_cv_gas)
              tempg(i,j,k)=xvb(i,j,k,4)/xvb(i,j,k,nv)*(pr*1.e-5)**(rg_over_cp_gas)
          enddo
         enddo
      enddo
      do ift=1,nfuel
        call updated(temps(ift,:,:,:),temps(ift,:,:,:),np,mp,l,1-ih,np+ih,1-ih,mp+ih,1,0)
      enddo
...
!     +        processor 0 is',1X,1pe17.10)
      if(irad.eq.1)then
       ...
       do j=1,mp
        do i=1,np
              ! FP minor fix here on kfuel computation,but irad.eq.1 not
              ! used for a while...
              if(sum(rhof(:,i,j,kreal)).le.min_rhof/nfuel) kfuel(i,j)=max(k-1,nz) 
        enddo
       enddo
      enddo
      endif
     ...
     &                 (sqrt(max((.23-xvb(i,j,kreal,7)/xvb(i,j,kreal,nv)),0.0)/.23)))
     &                 *xvb(i,j,kreal,nv)
         if (iradMultiFuelSimple.eq.1) then
            do ift=1,nfuel
              !if(rhof(ift,i,j,k).gt.min_rhof)
              if(rhof(ift,i,j,kreal).gt.min_rhof/nfuel) then !FP09/2019 BUG should be
                !kreal in rhof. CORRECTED!
                papvift(ift,i,j,k) = (2.0/sizescale(ift,i,j,kreal))
     &            *(rhof(ift,i,j,kreal)/(4.0*rhomicro(ift,i,j,kreal)))
                papv(i,j,k) = papv(i,j,k)+papvift(ift,i,j,k)
              endif
            enddo
         else ! version with different zones
           write(6,*) 'full version of multifuel for radiation not 
     ...
     +             +0.05*(1-xvb(i,j,kreal,7)/(0.23*xvb(i,j,kreal,nv)))
     +             -1.48e-4,0.0))**0.671 
           papvgas(i,j,k)=266.0*7.0*fvsoot*tempg(i,j,kreal)  
         endif
            if(irad.eq.1)then
              sk = nz+2
              if(papv(i,j,k).gt.papvmax) papvmax = papv(i,j,k)
              if(papvgas(i,j,k).gt.papvgasmax) papvgasmax = papvgas(i,j,k)
            else ! irad.eq.2
              sk = 1+2
         endif
...
!       if (mpi_rank.eq.0) close(21) 
      ...
      if (irad.eq.1) then
        call mpi_allreduce(papvmax,papvmax,1,mpi_real,mpi_sum,mpi_comm_world,ierror)
        call mpi_allreduce(papvgasmax,papvgasmax,1,mpi_real,mpi_sum,mpi_comm_world,ierror)
      else if(irad.eq.2)then
        do k=2,l  !papvtotAZ iradMultiFuelSimple>1 :check this
        do j=1,mp
         do i=1,np 
...
!!!----------------------- subroutine source defs -----------------------------------!!!
      ...
      !adjust kreal if using diffusion method (kreal indexes real domain arrays)
       ...
       do j=1,mp
        ...
        do i=1,np
         ...
         !define sourcesol 
            do ift=1,nfuel
              if((temps(ift,i,j,kreal).gt.(tambientarray(i,j,kreal)+rstep)))then
          !nbrterm accounts for neighbor influence on t4barsolid
              !FP 09/2019 correction of a bug, was tempg here initially
          if(k.gt.llowlim)then
                  frac=(1.0/6.0)*j3+.25*(1-j3)    !sets avg weight for 2-d or3-d
                  nbrterm = csurr*frac*abs(temps(ift,i,j,kreal)-temps(ift,i,j,kreal-1))
          else
                  frac=(1.0/5.0)*j3+.25*(1-j3)    !sets avg weight for 2-d or3-d
           nbrterm = 0.0
          endif
                yterm=abs(temps(ift,i,j,kreal)-temps(ift,i,j-j3,kreal))
     &            +abs(temps(ift,i,j,kreal)-temps(ift,i,j+j3,kreal))
           nbrterm = nbrterm+csurr*frac*
     &            (abs(temps(ift,i,j,kreal)-temps(ift,i-1,j,kreal))
     &            +abs(temps(ift,i,j,kreal)-temps(ift,i+1,j,kreal))
     &            +abs(temps(ift,i,j,kreal)-temps(ift,i,j,kreal+1))
     &        +yterm )
 
          !localterm accounts for local influence on t4barsolid
                localterm = camb*(temps(ift,i,j,kreal)-tambientarray(i,j,kreal))
          wdth=localterm+nbrterm
                t4barsolid=sqrt((temps(ift,i,j,kreal)**2+facp*wdth**2)
     &            *(temps(ift,i,j,kreal)**2+facm*wdth**2))
                sourcesol(ift,i,j,k)=4.0*papvift(ift,i,j,k)*pathcoef*
     &            (t4barsolid-tambientarray(i,j,kreal)**2)
     &            *(t4barsolid+tambientarray(i,j,kreal)**2)*
     &                     5.67051e-8
     ...
     &        +yterm )
          ...
          wdth=localterm+nbrterm
              t4bar=sqrt((tempg(i,j,kreal)**2+facp*wdth**2)
     &                     *(tempg(i,j,kreal)**2+facm*wdth**2))
          sourcegas(i,j,k)=4.0*papvgas(i,j,k)*
     &          (t4bar-tambientarray(i,j,kreal)**2)
     &          *(t4bar+tambientarray(i,j,kreal)**2)*
     &                     5.67051e-8
...
!        COMPUTATION OF SOURCE TERMS (EAZ, E2AZ, nphotemisAZ, nphotemisAZsplit)
      ...
      Etot = 0.                 !Etot= Total energy
        if (iradeastflux.eq.1) eastFlux=0.0
...
!      ENDIF
      ...
      DO k=2,l  ! FPAZ all real cell except the first one
       DO j=1,mp
        ...
        DO i=1,np
         ia=(npos-1)*np+i 
              EAZ(i,j,k+lphase) = (sourcegas(i,j,k) + sum(sourcesol(:,i,j,k)))
     +          *volumeAZ(i,j,k+lphase)
         Etot = Etot + EAZ(i,j,k+lphase)
        ENDDO !do i
       ENDDO !do j
      ENDDO !do k
      !FPAZ source computation in first real layer
        !FP092019 : here we will need a loop on lphase for
        !iradMultiFuel.eq.2
      DO j=1,mp
        ja=(mpos-1)*mp+j
        DO i=1,np
          ...
          !gas + solid in the first cell
            sourceSolSum=sum(sourcesol(:,i,j,1))
          if (nzonebelowcell1(i,j).eq.0) then
              EAZ(i,j,1) = (sourcegas(i,j,1)+sourceSolSum)*volumeAZ(i,j,1)
            !gas+solid in the second cell
              EAZ(i,j,2) = (sourcegas(i,j,1)+sourceSolSum)*volumeAZ(i,j,2)
            Etot = Etot + EAZ(i,j,1)+ EAZ(i,j,2)
          else
            !gas + solid in the first cell
              EAZ(i,j,1) = (sourcegas(i,j,1)+sourceSolSum)*volumeAZ(i,j,1)+
     +          sourceSolSum*volumeAZ(i,j,2)
...
!      use constants,only: ss,rhomicrovalue
      ...
      !call rmaxmin2(actualfueldepth,'actualfueldepth',1,np,1,mp,l)
      !call rmaxmin1(papv,'papv',1,np,1,mp,l)
      !call rmaxmin1(papvgas,'papvgas',1,np,1,mp,l)
      !call rmaxmin2(temps,'temps',1,np,1,mp,l)
      ...
      call rmaxmin2(-frhosiesrad, 'frhosiesrad', 1-ih, np+ih, 1-ih, mp+ih, l)
...
! this subroutine update values of firad and frhosiesrad and nphotonEastFlux according to nphotonAZtot and Qphot
      SUBROUTINE updateRadArrays(Qphot)
       ...
       IMPLICIT NONE
       INTEGER i, j, k,ift,   ia, ja !, j1,itmp,jtmp
       real :: papvsolreal
       real*8 :: Qphot
       real,EXTERNAL :: zcart
            ...
            if (iradMultiFuelSimple.eq.1) then
              do ift=1,nfuel
               if(rhof(ift,i,j,k).gt.min_rhof/nfuel) then
                frhosiesrad(ift,i,j,k) = -sourcesol(ift,i,j,k)
     &                             +1.0/volumeAZ(i,j,k+lphase)*Qphot* ! FP
     &            nphotonAZgather(ia,ja,k+lphase)*papvift(ift,i,j,k)/papvtot2AZ(ia,ja,k+lphase)
               endif
              enddo
            else ! version with different zones
              write(6,*) 'full version of multifuel for radiation not 
     ...
     +             (zcart(zedge(2),i,j)-zcart(zedge(1),i,j)))
           ...
           IF(papvtot2AZ(ia,ja,1) .GT. papvthresh)
     &        firad(i,j,1) = firad(i,j,1)+1.0/(volumeAZ(i,j,1)+volumeAZ(i,j,2))*
     &         Qphot*nphotonAZgather(ia,ja,1)*papvgas(i,j,1)/papvtot2AZ(ia,ja,1)
           IF(papvtot2AZ(ia,ja,2) .GT. papvthresh)
     &        firad(i,j,1) = firad(i,j,1)+1.0/(volumeAZ(i,j,1)+volumeAZ(i,j,2))*
     &         Qphot*nphotonAZgather(ia,ja,1+lphase)*papvgas(i,j,1)/papvtot2AZ(ia,ja,1+lphase)
            !solid phase
           if (iradMultiFuelSimple.eq.1) then
             do ift=1,nfuel
               if(rhof(ift,i,j,1).gt.min_rhof/nfuel) then
                 frhosiesrad(ift,i,j,1) = -sourcesol(ift,i,j,1) 
                 if (nzonebelowcell1(i,j).eq.0) then
                   frhosiesrad(ift,i,j,1) = frhosiesrad(ift,i,j,1)
     &                  +1.0/(volumeAZ(i,j,1)+volumeAZ(i,j,2))*
     &             Qphot*nphotonAZgather(ia,ja,1)*papvift(ift,i,j,1)/papvtot2AZ(ia,ja,1)
                   !k=1+lphase
                   frhosiesrad(ift,i,j,1) = frhosiesrad(ift,i,j,1)
     &               +1.0/(volumeAZ(i,j,1)+volumeAZ(i,j,2))*
     &               Qphot*nphotonAZgather(ia,ja,1+lphase)*papvift(ift,i,j,1)/papvtot2AZ(ia,ja,1+lphase)
       else !nzonebelowcell...
                   papvsolreal=papvift(ift,i,j,1)*(volumeAZ(i,j,1)+volumeAZ(i,j,2))/
     +      volumeAZ(i,j,1)
                   frhosiesrad(ift,i,j,1) = frhosiesrad(ift,i,j,1)
     &                 +1.0/(volumeAZ(i,j,1)+volumeAZ(i,j,2))*
     &       Qphot*nphotonAZgather(ia,ja,1)*papvsolreal/papvtot2AZ(ia,ja,1)
                 endif !nzonebelowcell
               endif !rhof.gt.minrhof
             enddo
           else !iradMultiFuel.eq.2 to be done
           endif
         ENDDO !do i
         ENDDO !do j
...
c     endif
     ...
     &                  (papvgas(i,j,kk)+papv(i,j,kk))*keff(i,j,k)
         ...
         if(k.le.kf)srcsol(i,j,k)=sum(sourcesol(:,i,j,k))
        enddo
       enddo
      enddo
     ...
     +               (srcsol(i,j,k)-sinksol(i,j,k)*gi0(i,j,k)*Es(i,j,k))
      ...
      do k=nz,Lnz
       ...
       do j=1,mp
        do i=1,np
         ! FP 09/2019 this has not be used for a while but looks
         ! incorrect should be checked in details but not done because
         ! not used anymore
         !firad(i,j,kreal)=firad(i,j,kreal)-rnetgas(i,j,k)
         ! frhosiesrad(i,j,kreal)=frhosiesrad(i,j,kreal)-rnetsol(i,j,k)
         STOP
!         if (papv(i,j,kreal).gt.0.0)
!     &    frhosiesrad(ift,i,j,kreal)=-(rnetsol(i,j,k)-srcsol(i,j,k))*
!     &        papvift(ift,i,j,kreal)/papv(i,j,kreal)
!         ! should be something like
!         firad(i,j,kreal)=-rnetgas(i,j,k)
        enddo !do i
       enddo !do j
      enddo !do k
