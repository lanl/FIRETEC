      ...
      real::rneteng,ff_sum,fw_sum  ! defined in fuel.f
      real::rhofold,rhovaporFrac
      
!      if(inonlocal.eq.1)then ! Function is way out-of-date and needs attention -AJJ
!        call firetecnonlocal(it)
!      else
      call rmaxmin2(temps,'temps',1-ih,np+ih,1-ih,mp+ih,l)
      ...
      if (iheatsource.ge.1.and.((time+restarttime).ge.startigntime)) call updateHeatSourcePosition()
...
!       do k=1,l
        do j=1,mp
          do i=1,np
            if (sum(rhof(:,i,j,k)).gt.min_rhof) then
c      begin firetec small timestepping loop
              do its=1,ntp
                ff_sum=0.0;fw_sum=0.0;rneteng=0.0
                rhofold=sum(rhof(:,i,j,k))
                rhovaporFrac=irhovapor*xvb(i,j,k,8)/xvb(i,j,k,nv)
...
cc ff and fw, psif, psiw psiwmax, frhof,frhowater thanks to the pdf function
                    call fuel(ift,i,j,k,rneteng,ff_sum,fw_sum,rhofold)
c compute frho, fox,frhovapor
      ...
      ! call rmaxmin(xvb,1-ih,np+ih,1-ih,mp+ih,l,nv)
       ! call rmaxmin2(firad,'firad',1-ih,np+ih,1-ih,mp+ih,l)
       ! call rmaxmin2(temps,'temps at end of FIRETEC',1-ih,np+ih,1-ih,mp+ih,l)
       ! call rmaxmin2(frhosiesrad,'frhosiesrad',1-ih,np+ih,1-ih,mp+ih,l)
       ! call rmaxmin2(firad,'firad',10,40,4,16,10)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real,intent(inout)::ff_sum,fw_sum !defined in fuel.f
      real:: gammaterm,rwatergainht,rnetmass
      real:: energy,capqterm
      real::fi,frho,fox,frhovapor
      frho=(ff_sum*rnfuel ! mass from combustion
     .      +fw_sum)*dtp ! mass from vaporisation 
      !dti is already in it (turb.f) fox=(-ff*rno+0.5*foxb(i,j,k)*dti)*dtp
      fox=(-ff_sum*rno+0.5*foxb(i,j,k))*dtp
      ...
      if(irhovapor.eq.1) then
        frhovapor=(fw_sum+0.5*frhovaporb(i,j,k) 
     +     +ff_sum*rnfuel*waterProductToWoodMassRatio)*dtp
         !fraction from combustion here (fraction of products*ff) 
        energy=energy+ 0.5 * frhovaporb(i,j,k) * (cv_vapor-
     +    cv_air)  ! vapor diffusion term
      endif
      !pr(i,j,k)=(xvb(i,j,k,4)*rg/prrcp)**(cp/cv)
      !capqterm=energy*(1.e5/pr(i,j,k))**(rg/cp)/cp !rrl
      !NB FP09/2019: no need for updateGasThermalProperties (done in
      !firetec before calling gas_dtp)
      pr(i,j,k)=(xvb(i,j,k,4)
     +     *rg_over_prrcp_gas)**(cp_over_cv_gas)
      capqterm=energy
     +     *(1.e5/pr(i,j,k))**(rg_over_cp_gas)/cp_gas !rrl
      ...
      if(irhovapor.eq.1) xvb(i,j,k,8)=xvb(i,j,k,8)+frhovapor
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real::fi,frho,fox,frhovapor
        ...
        !frho=0.0
        frho=0.5*foxb(i,j,k)*dt
        fox=0.5*foxb(i,j,k)*dt
        if(irhovapor.eq.1) then
          frhovapor=0.5*frhovaporb(i,j,k)*dt
          frho=frho+0.5*frhovaporb(i,j,k)*dt
        endif
        energy=firad(i,j,k)
        !NB FP09/2019: no need for updateGasThermalProperties (done in
        !firetec before calling gas_ltp)
        pr(i,j,k)=(xvb(i,j,k,4)
     +         *rg_over_prrcp_gas)**(cp_over_cv_gas)
        capqterm=energy
     +        *(1.e5/pr(i,j,k))**(rg_over_cp_gas)/cp_gas !rrl
...
c       BELOW THIS LINE : IGNITION ONLY       
       ...
       if(igntype==2.and.((time+restarttime).lt.endigntime))then     !terratorch style ignition
        ...
        travelrate=sqrt((xfirelinelow-xfirelinehigh)**2+(yfirelinelow-yfirelinehigh)**2)/rignitionperiod
        pretime=flamedistance/travelrate
        if(pretime.gt.startigntime)then
         startigntime=pretime
         endigntime=pretime+(rignitionperiod)
        endif
        if((time+restarttime).ge.startigntime-pretime.and.(time+restarttime).le.endigntime+pretime) then
              ...
              do i=1,np
                ...
                do ift=1,nfuel
                  if(y1.ge.yfirelinelow.and.y1.le.yfirelinehigh.and.k.lt.4.)then
                    ...
                    tempsold=temps(ift,i,j,k)
                    temps(ift,i,j,k)=max(temps(ift,i,j,k),min(targettemp,
     &                          targettemp-targettemp/flamedistance*dist))
                    if(temps(ift,i,j,k).ne.tempsold)then
                      cpsolid(ift,i,j,k)=cpwood
                      sies(ift,i,j,k)=cpsolid(ift,i,j,k)*temps(ift,i,j,k)
                      rhowater(ift,i,j,k)=0.0
                      rmoist(ift,i,j,k)=0.0
                      rhos(ift,i,j,k)=rhof(ift,i,j,k)
                    endif
                  endif
                enddo
              enddo
          ...
          do j=1,mp
            ...
            !gridlist
                  if (igniteVerticalExtent.eq.0.or.k.le.igniteVerticalExtent) then
                    if(rhof(ift,i,j,k).GT.min_rhof/nfuel) then   
                      temps(ift,i,j,k)=max(temps(ift,i,j,k),min(((time+restarttime)-startigntime)
...
! JLW 05/09/13 
     ...
     &        (time+restarttime.le.igntime(iaerial)+(targettemp-tambient)/ramprate)) then
             do j=1,mp
               do i=1,np
                 ia=(npos-1)*np+i
                 ja=(mpos-1)*mp+j
                  if(ia.eq.ignloc(iaerial,1).and.ja.eq.ignloc(iaerial,2)) then
                   do jj=j,j+1
                     do ii=i,i+1
                        do ift=1,nfuel
                          temps(ift,ii,jj,1)=max(temps(ift,ii,jj,1),min(((time+restarttime)-
     &                      igntime(iaerial))*ramprate+tambient,targettemp))
                          cpsolid(ift,ii,jj,1)=cpwood
                          sies(ift,ii,jj,1)=cpsolid(ift,ii,jj,1)*temps(ift,ii,jj,1)
                          rhowater(ift,ii,jj,1)=0.0
                          rmoist(ift,ii,jj,1)=0.0
                          rhos(ift,ii,jj,1)=rhof(ift,ii,jj,1)
                        enddo
                     enddo
                   enddo
                 endif
...
! JLW 05/09/13  
       elseif(igntype==5) then                         ! atv/driptorch ignition
        if (time+restarttime.lt.atvtime(natv,2)) then
         do iatv=1,natv
            if ((time+restarttime.ge.atvtime(iatv,1)).and.(time+restarttime.le.atvtime(iatv,2))) then
              finterp=((time+restarttime)-atvtime(iatv,1))/(atvtime(iatv,2)-atvtime(iatv,1))
            do j=1,mp
              do i=1,np
                ia=(npos-1)*np+i
                ja=(mpos-1)*mp+j
                x1=(float(ia)-.5)*dx
                y1=(float(ja)-.5)*dy
                  x2=(atvstop(iatv,1)-atvstart(iatv,1))*finterp+atvstart(iatv,1)
                  y2=(atvstop(iatv,2)-atvstart(iatv,2))*finterp+atvstart(iatv,2)
                dist=sqrt((x2-x1)**2+(y2-y1)**2)
                  do ift=1,nfuel
                    tempsold=temps(ift,i,j,1)
                    temps(ift,i,j,1)=max(temps(ift,i,j,1),min(targettemp,
     &                    targettemp-targettemp/flamedistance*dist))
                    if(temps(ift,i,j,1).ne.tempsold)then
c       print *,'atv ',time,atvtime(iatv,1),x1,y1,x2,y2,dist,
c    &                temps(i,j,1),tempsold,finterp,i,j,
c    &                mpi_rank,targettemp,flamedistance
                      cpsolid(ift,i,j,1)=cpwood
                      sies(ift,i,j,1)=cpsolid(ift,i,j,1)*temps(ift,i,j,1)
                      rhowater(ift,i,j,1)=0.0
                      rmoist(ift,i,j,1)=0.0
                      rhos(ift,i,j,1)=rhof(ift,i,j,1)
                endif
              enddo
            enddo
              enddo
        ...
        rignitionperiod=endigntime1-startigntime1
          travelrate=sqrt((xlineor1-xlineend1)**2+(ylineor1-ylineend1)**2)/rignitionperiod
        pretime=flamedistance/travelrate
     ...
     &     (time+restarttime).le.endigntime1+pretime) then
           ...
           do i=1,np
            ...
            if(y1.ge.min(ylineor1,ylineend1).and.y1.le.max(ylineor1,ylineend1).and.k.lt.2)then
             dist=sqrt((x2-x1)**2+(y2-y1)**2)
                    do ift=1,nfuel
                      tempsold=temps(ift,i,j,k)
                      temps(ift,i,j,k)=max(temps(ift,i,j,k),min(targettemp,(targettemp-tambient)*(1.-dist/flamedistance)+tambient))
                      if(temps(ift,i,j,k).ne.tempsold)then
                        cpsolid(ift,i,j,k)=cpwood
                        sies(ift,i,j,k)=cpsolid(ift,i,j,k)*temps(ift,i,j,k)
                        rhowater(ift,i,j,k)=0.0
        ...
        rignitionperiod=endigntime2-startigntime2
          travelrate=sqrt((xlineor2-xlineend2)**2+(ylineor2-ylineend2)**2)/rignitionperiod
        pretime=flamedistance/travelrate
     ...
     +           sqrt((x2-x1)**2+(y2-y1)**2)
            if ((ps1>=0.and.ps2>=0.and.dist<=flamedistance).or.dist1<=flamedistance.or.dist2<=flamedistance) then
                do ift=1,nfuel
                  if (temps(ift,i,j,k)<targettemp) then
        !     write(6,*) ia,ja,temps(i,j,k),dist
                    temps(ift,i,j,k)=temps(ift,i,j,k)+(targettemp-tambient)*0.01
                    cpsolid(ift,i,j,k)=cpwood
                    sies(ift,i,j,k)=cpsolid(ift,i,j,k)*temps(ift,i,j,k)
                    rhowater(ift,i,j,k)=0.0
                    rmoist(ift,i,j,k)=0.0
...
c       - if ihsmass=1 there is also a source of mass in k.eq.1
     ...
     +    /(dx*dy*(zcart(zedge(lHeatSource+1),i,j)-zcart(zedge(1),i,j)))
         ...
         !NB FP09/2019: no need for updateGasThermalProperties (done in
         !firetec before calling heatSourceRHS)
         pr(i,j,k)=(xvb(i,j,k,4)
     +          *rg_over_prrcp_gas)**(cp_over_cv_gas)
         capqterm=energy
     +         *(1.e5/pr(i,j,k))**(rg_over_cp_gas)/cp_gas 
