c xevariation takes into account any evolution of xe 
     ...
     &               xvbdatanew(1,1,1,2)
        ...
        do kv=1,nv
          call updated(xe(1-ih,1-ih,1,kv),xe(1-ih,1-ih,1,kv),np,mp,l,1-ih,np+ih,1-ih,mp+ih,0)
        enddo
        ...
        if(uswitch==1)then
          do k=1,l
            do j=1,mp
              do i=1,np
                if(uramp.GE.0.0) then
!                xe(i,j,k,1)=min(uramp*xe(i,j,k,nv),
!     &          ((time+restarttime)/uramptime*uramp+u0)*xe(i,j,k,nv))
!     &                    *max(0.,(1.0-1.5*rhof(i,j,k)))
!     &                    *(((zcart(z(k),1,1)-zs(1,1))/10.0)**(1.0/7.0))
!!     &                   *(min(100.0,zcart(z(k),1,1))/10.0)**(1.0/7.0)
! FIXME KOO
                xe(i,j,k,1)=min(uramp*xe(i,j,k,nv),
     &                      ((time+restarttime)
     &                      /(uramptime*uramp+0.000000001)+u0)*
     &                      xe(i,j,k,nv))
     &                      *max(0.,(1.-1.5*rhof(i,j,k)))
     &                      *(min(100.0,zcart(z(k),1,1))/10.0)**(1.0/7.0)

                 else
                xe(i,j,k,1)=max(uramp*xe(i,j,k,nv),
     &          ((time+restarttime)/uramptime*uramp+u0)*xe(i,j,k,nv))
     &                    *max(0.,(1.0-1.5*rhof(i,j,k)))
     &                    *(((zcart(z(k),1,1)-zs(1,1))/10.0)**(1.0/7.0))
                 endif 
        ...
        if(vswitch==1)then
          do k=1,l
            do j=1,mp
              do i=1,np
                if(vramp.GE.0.0) then 
                xe(i,j,k,2)=min(vramp*xe(i,j,k,nv),
     &         ((time+restarttime)/vramptime*vramp+v0)*xe(i,j,k,nv))
     &                      *max(0.,(1.-1.5*rhof(i,j,k)))
     &                    *(((zcart(z(k),1,1)-zs(1,1))/10.0)**(1.0/7.0))
!     &                      *(min(100.0,zcart(z(k),1,1))/10.0)**(1.0/7.0)
                else
                xe(i,j,k,2)=max(vramp*xe(i,j,k,nv),
     &         ((time+restarttime)/vramptime*vramp+v0)*xe(i,j,k,nv))
     &                      *max(0.,(1.-1.5*rhof(i,j,k)))
     &                    *(((zcart(z(k),1,1)-zs(1,1))/10.0)**(1.0/7.0))
!     &                      *(min(100.0,zcart(z(k),1,1))/10.
                 endif  
              enddo
            enddo
          enddo
