      ...
      Implicit None
       ...
       else ! restart from a windfieldtart (irst.eq.0 and iwindfieldin.eq.1)
        itrestart=0
        if(mpi_rank.eq.0)write(6,*)'starting with windfieldstart :',windfieldstartfile
        call irstreadio2(windfieldstartfile)
       end if 
c        do kv =1,nv
c           call updated(xvb(1-ih,1-ih,1,kv),xe(1-ih,1-ih,1,kv),np,mp,l,1-ih,np+ih,1-ih,mp+ih,1)
c        enddo
...
! KOO added for Valles
       if(iwindfieldin.eq.2)then
        do kv=1,nv
         do k=1,l
          do j=1,mp
            do i=1,np
              xe(i,j,k,kv)=xvb(i,j,k,kv)
            enddo
          enddo
        enddo
       enddo
