...
!----------------------------------------------------------
     ...
     +                         status='unknown')
      ...
      call writeio(xvb(1-ih,1-ih,1,2),21,1-ih,np+ih,1-ih,mp+ih,l,0) ! field 2
      ...
      if (ilspgf.ge.2) call writeio(sintheta,21,1-ih,np+ih,1-ih,mp+ih,l,0) 
      if(irod.eq.1) then
...
!----------------------------------------------------------
     ...
     + open(41,file=fname,form='unformatted',status='old')
      do ir=1,nrst
        ...
        if (ilspgf.ge.2) call readio(sintheta,41,l,1-ih,np+ih,1-ih,mp+ih) 
...
c
     ...
     .         front=real(i)
       ...
       if((iflag.eq.2.or.iflag.eq.3).and.iwindfieldout.eq.1) then
       u1=0.0
       u2=0.0
       umean=0.0
       do k=1,l-1
        ...
        do ja=1,m
         do ia=1,n
           u1=u1+outdata(ia,ja,k)
           u2=u2+outdata(ia,ja,k+1)
         enddo
        enddo
         umean=(u1*(z2-zu)+u2*(zu-z1))
     +    /(n*m*(z2-z1))
        endif
         if (iflag.eq.2) write(6,*) 'rho*u at height ',zu, ' is ',umean
         if (iflag.eq.3) write(6,*) 'target rhou is',u0*umean
       enddo
       endif
