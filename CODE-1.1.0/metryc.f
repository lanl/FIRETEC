...
c get spline coefficients for ginverse and dgdsigma
      ...
      call updated(gi,gi,np,mp,l,1-ih,np+ih,1-ih,mp+ih,1)
      call updated(h,h,np,mp,l,1-ih,np+ih,1-ih,mp+ih,1)
      call updated(go,go,np,mp,1,1-ih,np+ih,1-ih,mp+ih,1)
   ...
   51 c23(i,j)=.5*dyi*(1./go(i,j+j3)-1./go(i,j-j3))*go(i,j)
     ...
     .        go(i,mp+1)
      ...
      call updated(c13,c13,np,mp,1,1-ih,np+ih,1-ih,mp+ih,1)
      call updated(c23,c23,np,mp,1,1-ih,np+ih,1-ih,mp+ih,1)
      deallocate (go)
...
! sigma0=aa3**sigma**3+aa2*sigma**2+aa1*sigma   ! cubic polynomial fit
      ...
      if(aa1.eq.0)then           ! use spline
         ...
         if(iflag.eq.0)gdeform=aa3*sigmax**3+aa2*sigmax**2+aa1*sigmax
         if(iflag.eq.1)gdeform=3*aa3*sigmax**2+2*aa2*sigmax+aa1
      endif
      return
