...
!!!--------------------------subroutine frad_init  -----------------------------------!!!
      ...
      if(irad.eq.2)then 
       ...
       allocate(nphotemisAZ(n,m,l+lphase))       ! big array  #1 
       allocate(nphotemisAZsplit(n,m,l+lphase))             ! #2
       allocate(nphotemisAZtemp(n,m,l+lphase))              ! #3
       allocate(nphotonAZgather(n,m,l+lphase))              ! #4
       allocate(nphotonAZ(n,m,l+lphase))                    ! #5
       if (iradeastflux.eq.1) then
           allocate(nphotonEastFlux(n,m,l+lphase))          ! #6
           allocate(nphotontempEastFlux(n,m,l+lphase))      ! #7
           nphotonEastFlux=0
           nphotontempEastFlux=0
       endif
       allocate(EAZ(np,mp,l+lphase))
       allocate(E2AZ(n,m,l+lphase))                         ! #8
       allocate(percabsAZold(n,m,l+lphase))                 ! #9
       allocate(percabsAZnew(n,m,l+lphase))                ! #10
      ...
      if (iinra.eq.0) then
         crad=50.0 ! 20       !coefficient for papv calculations in radiation 
      else
      ...
      enddo
      enddo
       call updated(gi0,gi0,np,mp,LL,1-ih,np+ih,1-ih,mp+ih,1)
       ...
       enddo
       ! FP added l+1
       call updated(zcoordsf,zcoordsf,np,mp,l+1,1-ih,np+ih,1-ih,mp+ih,1)
       call stretch_keff()
...
!!!----------------------- subroutine papv_defs -----------------------------------!!!
      ...
      !calculate current tempgas  (moved from source_defs because required by new soot model
        do k=1,l
         ...
         enddo
        enddo
      call updated(temps,temps,np,mp,l,1-ih,np+ih,1-ih,mp+ih,1)
...
c     endif
     ...
     +                   mpi_comm_world,ierror)
      call updated(surface,surface,n,m,1,1-ih,n+ih,1-ih,m+ih,1)
