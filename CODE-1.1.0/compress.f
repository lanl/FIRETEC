...
!-------------------------------------------------------------------------
      ...
      integer :: nplwrites,i,j,k,it, inv,ia,ja !kv 
      ...
      character(len=60)::pfilename ,fname
      integer :: ierr 

! KOO Valles special case
      integer :: icenterx, icentery!  rand_seed_valles 
      real :: radius_valles, arand_valles
...
c
     ...
     .irst,nrst,nt,nts,
     .isplit,nprocx,nprocy,n,m,l,nv,nv2d,dx,dy,dz,dts,ntp,
     .ih,nr,ibcx,ibcy,irlx,irly,ibclatopen,ibctopopen,iab,
     .zab,zabt,tow,itheta,
     .irod,iturb,idrag,isa,ilapdo,
     .irad,icallrad,irandseed,iseed,
     .inonlocal,isubgridgas,  !FP
     .irhovapor,icorio,ilspgf,frqlspgf,islip,iperturb,
     .u0,uramp,uramptime,uswitch,zu,
     .v0,vramp,vramptime,vswitch,
     .slopeangle,slopeazimuth,
     .st,iord,isor,nonos,idiv,nfct,nonosold,
     .ifuelinra,ifuel,idirt,ivegread,kmax,kf77max,frqoutput,outname,
     .restartfile,ioextra,topofile,ipotflow,
     .isoturb,ibctopbot,ignfile,icfmeflag,
     & windfieldstartfile,xvbdataname,
     +iwindfieldout,iwindfieldin,itwindfield, !JMC 10/7/6
     &itinterp,ibcells,jbcells,  !JMC 10/7/6
     .is,ie,js,je,
     .iunstable,
     .iwallclock,                              !KOO 07/16/07
     .ifbrand,                                 !KOO
     .ilocation, iinra, irrl, ijww,ijas,ijmc,ieko,ifp,ijld,iekl
...
c     ! list of personal parameter for fp
     ...
     .ist, iradeastflux,ius,iue,jus,jue,windspeedupfactor
!   
...
!
      ...
      call MPI_Bcast(nv2d,1,mpi_integer,0,mpi_comm_world,ierr)
      ...
      call MPI_Bcast(ilocation,1,mpi_integer,0,mpi_comm_world,ierr)
      call MPI_Bcast(iinra,1,mpi_integer,0,mpi_comm_world,ierr)
...
! personal flag and personal parameters
      call MPI_Bcast(irrl,1,mpi_integer,0,mpi_comm_world,ierr)
      call MPI_Bcast(ijww,1,mpi_integer,0,mpi_comm_world,ierr)
      call MPI_Bcast(ijmc,1,mpi_integer,0,mpi_comm_world,ierr)
      call MPI_Bcast(ijas,1,mpi_integer,0,mpi_comm_world,ierr)
      call MPI_Bcast(ijmc,1,mpi_integer,0,mpi_comm_world,ierr)
      call MPI_Bcast(ieko,1,mpi_integer,0,mpi_comm_world,ierr)
      call MPI_Bcast(ifp,1,mpi_integer,0,mpi_comm_world,ierr)
      call MPI_Bcast(ijld,1,mpi_integer,0,mpi_comm_world,ierr)
      call MPI_Bcast(iekl,1,mpi_integer,0,mpi_comm_world,ierr)
      call MPI_Bcast(fuelnumber,1,mpi_integer,0,mpi_comm_world,ierr)
...
! NaN check 
!             do inv=1,nv
!               if(xvb(i,j,k,inv).NE.xvb(i,j,k,inv)
!     +            .or. xvb(i,j,k,inv).EQ.xvb(i,j,k,inv)+1.0
!     +           ) then
!                 ia=(npos-1)*np+i
!                 ja=(mpos-1)*mp+j
!                 print*,'NaN_xvb:',ia,ja,k,inv,mpi_rank
!                 STOP
!               endif 
!              enddo

! KOO Valles, remove fuels when ittot=900. BEGINS here FIXME later
!           if (ittot.eq.320) then 
!             if(i+j+k.eq.3) then 
!              if(mpi_rank.eq.0) print*,'Valles special case'
!              if(mpi_rank.eq.0) print*,'removing fuels behind fire line'
!!              call random_seed(mpi_rank)
!             endif 
!             ia=(npos-1)*np+i
!             ja=(mpos-1)*mp+j
!             if (sqrt(real((ia-icenterx)**2)+real((ja-icentery)**2))
!     +          .LT. radius_valles) then 
!                call random_number(arand_valles)         
!                arand_valles=arand_valles*0.999999 
!!                if(i.eq.np .and. j.eq.mp) then 
!!                   print*,'random numbers',mpi_rank,arand_valles 
!!                endif 
!                if (k.eq.1) then 
!                   if(arand_valles.LE.0.8)then 
!                      rhof(i,j,k)=1.0e-6 
!                   endif 
!                else ! k.eq.1
!                   if(arand_valles.LE.0.6) then 
!                      rhof(i,j,k)=1.0e-6
!                   endif 
!                endif  !k.GT.1 
!             endif  ! within radius_valles 
!           endif ! ittot==320 

! KOO Valees, remove fuels when ittot=900. ENDS here 
            ...
            uavg(i,j,k)=wt(1)*xvb(i,j,k,1)/xvb(i,j,k,nv)
            vavg(i,j,k)=wt(1)*xvb(i,j,k,2)/xvb(i,j,k,nv)
            wavg(i,j,k)=wt(1)*xvb(i,j,k,3)/xvb(i,j,k,nv)

          if(xvb(i,j,k,7)/xvb(i,j,k,8).GT.0.22
     +     .or. xvb(i,j,k,4)/xvb(i,j,k,8).LT.250) then
...
c walltimer - JMC1 !KOO
      ...
      if(iwallclock.EQ.1) call computeWallTime(wtime2,4,'advel')

      call advec(xvb,1-ih,np+ih,1-ih,mp+ih,l,nv)  ! calling mpdata
...
c        if(it.gt.1)write (*,*) 'rkmax violation 6, rkmax=',xvb(i,j,k,6)
      ...
      call updated(xvb(1-ih,1-ih,1,5),xe(1-ih,1-ih,1,5),np,mp,l,1-ih,np+ih,1-ih,mp+ih,1)
      call updated(xvb(1-ih,1-ih,1,6),xe(1-ih,1-ih,1,6),np,mp,l,1-ih,np+ih,1-ih,mp+ih,1)
      endif
...
c      endif
      if (iwallclock.EQ.1)wtime2=MPI_Wtime()
       ...
       if (ilspgf.ge.2.and.it/frqlspgf*frqlspgf.eq.it) call largeScalePGF()
