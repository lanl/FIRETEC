...
subroutine tinit
  ...
  real(prec),allocatable :: relb(:)

  ! Executable Code
  allocate(relb(nr+1)); relb = 0.0
  ...
  if(ibcx.ge.1.or.ibcy.ge.1)then
    ...
    select case(ibcx)
      case(4)
        do i=1,nr+1
          relb(i)=exp(-dble(i)*(dble(i)-1./dble(i))**2./(2.*dble(nr-1)**2.))
          !relb(i)=1.0/(tow*(1.+(dble(i)/(0.5*dble(nr)-1.))**2.))
        enddo
      case(3)
        do i=1,nr+1
          relb(i)=exp(-8.*dble(i-1)/dble(nr-1))
        enddo
      case(2)
        do i=1,nr+1
          relb(i)=1.5*0.25/(tow*(1.+(dble(i)/(0.5*dble(nr)-1.))**2.))
        enddo
      case(1)
        do i=1,nr+1
          relb(i)=0.05*exp(-dble(i)*(dble(i)-1./dble(i))**2./(2.*dble(nr-1)**2.))
        enddo
    end select
#else
    select case(ibcx)
      case(4)
        do i=1,nr+1
          relb(i)=exp(-real(i)*(real(i)-1./real(i))**2./(2.*real(nr)**2.))
          !relb(i)=1.0/(tow*(1.+(real(i)/(0.5*real(nr)-1.))**2.))
        enddo
      case(3)
        do i=1,nr+1
