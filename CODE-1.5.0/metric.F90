subroutine metric()
  use gridlist_variables, only : aa1,l,ih,dx,dy,prec
  ...
  integer :: i,j,k
  real(prec) :: f
  real(prec) :: gdeform,gdeform1
  real(prec),allocatable :: go(:,:)

  ! Executable code
  allocate (go(1-ih:np+ih,1-ih:mp+ih)); go = 0.0
...
module zcart_function
  use gridlist_variables, only : prec

  contains

  real(prec) function zcart(sigmax,i,j)
