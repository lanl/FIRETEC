subroutine filstr()
  use gridlist_variables, only : l,ih,frqfilstr,prec
  use xvall, only : xv,xe
  use gridsetup, only : np,mp
  Implicit None
  real(prec),allocatable:: atmpx(:), atmpy(:), atmpz(:)
  integer :: i,j,k,kv
  real(prec) ::f1,f2
  ...
  !up filtering
  f1=1.0-f2
  allocate (atmpx(np)); atmpx=0.0
  allocate (atmpy(mp)); atmpy=0.0
  allocate (atmpz(l)) ; atmpz=0.0
