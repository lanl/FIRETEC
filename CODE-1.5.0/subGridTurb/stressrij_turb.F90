...
subroutine stressrij(fx,fy,fz,il,iu,jl,ju,lls)
  ...
  use linn_turb_variables, only : sqrtG_Kxy,sqrtG_Kz,sqrtG_KG33,rtke_abc
  ...
  real(prec),intent(inout) :: fx(il:iu,jl:ju,lls)
  real(prec),intent(inout) :: fy(il:iu,jl:ju,lls)
  real(prec),intent(inout) :: fz(il:iu,jl:ju,lls)

  integer :: i,j,k,kp1,kk,km1
  real(prec) hdxi,hdyi,hdzi
  real(prec):: J31,J32 ! jacobians
  ! cell face quantities for Aijc
  real(prec):: sqrtG_KG11a,sqrtG_KG22a, sqrtG_KG13a,sqrtG_KG31a, &
   sqrtG_KG23a,sqrtG_KG32a, sqrtG_KG33a 
  ! cell face for Bijc
  real(prec) :: sqrtG_Kxya &
     ,sqrtG_Kxy_J31_J31a, sqrtG_Kxy_J31_J32a,&
     sqrtG_Kxy_J32_J32a, Kza,Kz_J31a, Kz_J32a, Kxya,Kz_J33a   
  ! cell face model derivative
  real(prec)::ux,uy,uz,vx,vy,vz,wx,wy,wz
  ! contravariant coordinates of Aij= Kj*dui/dxj
  real(prec)::A11c,A12c,A13c,A21c,A22c,A23c,A31c,A32c,A33c
  ! contravariant coordinates of Bij= Kj*duj/dxi
  real(prec)::B11c,B12c,B13c,B21c,B22c,B23c,B31c,B32c,B33c
  real(prec)::t11a,t12a,t13a,t21a,t22a,t23a,t31a,t32a,t33a 
  !  C=2/3*(rtkte_abc+k1*du/dx+k2*dv/dy+k3*dw/dz)
  ! cell face quantity for C
  real(prec):: rtke_abca, Kxy_uxa, Kxy_vya, Kz_wza,Kxy_J31a,Kxy_J32a
  ! C arrays at i-1/2 (Ci), j-1/2 (Cj), k-1/2 (Ck)
  !real, allocatable::Ci(:,:,:),Cj(:,:,:),Ck(:,:,:)
  real(prec)::Cx,Cy,Cz ! C model grid derivatives
  real(prec)::fu,fv,fw ! contribution of reynold stres to momentum u, v,w
  real(prec),allocatable :: AB11c(:,:,:),AB12c(:,:,:),AB13c(:,:,:)
  real(prec),allocatable :: AB21c(:,:,:),AB22c(:,:,:),AB23c(:,:,:)
  real(prec),allocatable :: AB31c(:,:,:),AB32c(:,:,:),AB33c(:,:,:)
  real(prec),allocatable :: Ci(:,:,:),Cj(:,:,:),Ck(:,:,:)
  ...
  do k=1,lls
    ...
    do j=1,mp
      do i=1,np+1
        ...
        uy = 0.5*(xvrho(i,j+1,k,iuvel)-xvrho(i,j-1,k,iuvel)+ &
          xvrho(i-1,j+1,k,iuvel)-xvrho(i-1,j-1,k,iuvel))*hdyi
        vy = 0.5*(xvrho(i,j+1,k,ivvel)-xvrho(i,j-1,k,ivvel)+ &
          xvrho(i-1,j+1,k,ivvel)-xvrho(i-1,j-1,k,ivvel))*hdyi
        uz = 0.5*(xvrho(i,j,kp1,iuvel)-xvrho(i,j,k-1,iuvel)+ &
          xvrho(i-1,j,kp1,iuvel)-xvrho(i-1,j,k-1,iuvel))*hdzi
        vz = 0.5*(xvrho(i,j,kp1,ivvel)-xvrho(i,j,k-1,ivvel)+ &
          xvrho(i-1,j,kp1,ivvel)-xvrho(i-1,j,k-1,ivvel))*hdzi
        wz = 0.5*(xvrho(i,j,kp1,iwvel)-xvrho(i,j,k-1,iwvel)+ &
          xvrho(i-1,j,kp1,iwvel)-xvrho(i-1,j,k-1,iwvel))*hdzi
  ...
  do k=1,lls
    ...
    do j=1,mp+1
      do i=1,np
        ...
        ux = 0.5*(xvrho(i+1,j,k,iuvel)-xvrho(i-1,j,k,iuvel)+ &
          xvrho(i+1,j-1,k,iuvel)-xvrho(i-1,j-1,k,iuvel))*hdxi
        vx = 0.5*(xvrho(i+1,j,k,ivvel)-xvrho(i-1,j,k,ivvel)+ &
          xvrho(i+1,j-1,k,ivvel)-xvrho(i-1,j-1,k,ivvel))*hdxi
        ...
        uz = 0.5*(xvrho(i,j,kp1,iuvel)-xvrho(i,j,k-1,iuvel)+ &
          xvrho(i,j-1,kp1,iuvel)-xvrho(i,j-1,k-1,iuvel))*hdzi
        vz = 0.5*(xvrho(i,j,kp1,ivvel)-xvrho(i,j,k-1,ivvel)+ &
          xvrho(i,j-1,kp1,ivvel)-xvrho(i,j-1,k-1,ivvel))*hdzi
        wz = 0.5*(xvrho(i,j,kp1,iwvel)-xvrho(i,j,k-1,iwvel)+ &
          xvrho(i,j-1,kp1,iwvel)-xvrho(i,j-1,k-1,iwvel))*hdzi
        ...
        B32c = t23a * vz    ! the others are 0 cause J21=J23=0 
  ...
  do k=1,lls+1
    ...
    do j=1,mp
      do i=1,np
        ...
        Kxya = 0.5*(sqrtG_Kxy(i,j,kk) * gi(i,j,kk) +  &
          sqrtG_Kxy(i,j,km1) * gi(i,j,km1))     
        Kza = 0.5*(sqrtG_Kz(i,j,kk) * gi(i,j,kk) +  &
          sqrtG_Kz(i,j,km1) * gi(i,j,km1))     
        ...
        Kz_J33a = 0.5*(sqrtG_Kz(i,j,kk)*gi(i,j,kk)**2+sqrtG_Kz(i,j,km1)&
            * gi(i,j,km1)**2)  
        sqrtG_Kxy_J31_J31a=0.5*(sqrtG_Kxy(i,j,kk)* &
          (c13(i,j)*gmul(kk))**2+sqrtG_Kxy(i,j,km1)* &
          (c13(i,j)*gmul(km1))**2) 
        sqrtG_Kxy_J31_J32a=0.5*(sqrtG_Kxy(i,j,kk)*c13(i,j)*c23(i,j)* &
          (gmul(kk))**2+sqrtG_Kxy(i,j,km1)*c13(i,j)*c23(i,j)* &
          (gmul(km1))**2)
        sqrtG_Kxy_J32_J32a=0.5*(sqrtG_Kxy(i,j,kk)* &
          (c23(i,j)*gmul(kk))**2+sqrtG_Kxy(i,j,km1)* &
          (c23(i,j)*gmul(km1))**2)
        ...
        ux = 0.5*(xvrho(i+1,j,kk,iuvel)-xvrho(i-1,j,kk,iuvel)+ &
          xvrho(i+1,j,k-1,iuvel)-xvrho(i-1,j,k-1,iuvel))*hdxi
        vx = 0.5*(xvrho(i+1,j,kk,ivvel)-xvrho(i-1,j,kk,ivvel)+ &
          xvrho(i+1,j,k-1,ivvel)-xvrho(i-1,j,k-1,ivvel))*hdxi
        wx = 0.5*(xvrho(i+1,j,kk,iwvel)-xvrho(i-1,j,kk,iwvel)+ &
          xvrho(i+1,j,k-1,iwvel)-xvrho(i-1,j,k-1,iwvel))*hdxi
        uy = 0.5*(xvrho(i,j+1,kk,iuvel)-xvrho(i,j-1,kk,iuvel)+ &
          xvrho(i,j+1,k-1,iuvel)+xvrho(i,j-1,k-1,iuvel))*hdyi
        vy = 0.5*(xvrho(i,j+1,kk,ivvel)-xvrho(i,j-1,kk,ivvel)+ &
          xvrho(i,j+1,k-1,ivvel)+xvrho(i,j-1,k-1,ivvel))*hdyi
        wy = 0.5*(xvrho(i,j+1,kk,iwvel)-xvrho(i,j-1,kk,iwvel)+ &
          xvrho(i,j+1,k-1,iwvel)+xvrho(i,j-1,k-1,iwvel))*hdyi
    ...
    do j=1,mp
      do i=1,np  
        ...
        fu = gi(i,j,k)*((AB11c(i+1,j,k)-AB11c(i,j,k))*dxi + & 
           (AB12c(i,j+1,k)-AB12c(i,j,k))*dyi +  &
           (AB13c(i,j,k+1)-AB13c(i,j,k))*dzi) - Cx - J31 * Cz      
        fv = gi(i,j,k)*((AB21c(i+1,j,k)-AB21c(i,j,k))*dxi + &
           (AB22c(i,j+1,k)-AB22c(i,j,k))*dyi + &
           (AB23c(i,j,k+1)-AB23c(i,j,k))*dzi) - Cy - J32 * Cz      
        fw = gi(i,j,k)*((AB31c(i+1,j,k)-AB31c(i,j,k))*dxi + &
           (AB32c(i,j+1,k)-AB32c(i,j,k))*dyi + &
           (AB33c(i,j,k+1)-AB33c(i,j,k))*dzi) - gi(i,j,k) * Cz          
        fx(i,j,k)=fx(i,j,k)+fu
        fy(i,j,k)=fy(i,j,k)+fv
        fz(i,j,k)=fz(i,j,k)+fw
      enddo
    enddo
  enddo
  
  deallocate(AB11c,AB12c,AB13c)
  deallocate(AB21c,AB22c,AB23c)
  deallocate(AB31c,AB32c,AB33c)
  deallocate(Ci,Cj,Ck)
