...
c    itdsource=6 -> Angel Fire tree data from Sandia (25 X 10)
        parameter (itdsource=3)
c
c  rotate tree data (0 => no, 1 => yes - so, if 20m X 50m data,
c  0 => 50 aligns with Y axis, 1=> 50 aligns with X axis)
        parameter (itdrotate=0)
c
c  itdloc=0 => spread tree data in random locations within any nxtd X
c              nytd section of the HIGRAD grid
c  itdloc=1 => spread tree data in uniform nxtd X nytd patterns 
c  itdloc=2 => read tree locations and information from a previous run 
c              of this program from file oldtreefile (original sampled/
c              measured data file will also be needed)
...
c  arrays for HIGRAD data
        ...
        real rhoflitter(nxh,nyh,nzh)   ! array for saving litter contrib
...
c  indicates areas to be thinned is being used
        parameter(ithinminxh=1,ithinmaxxh=500,ithinminyh=1,
     +                                            ithinmaxyh=500) 
c
...
c
      ...
      use rhoinfo
c
      real x(nxh,nyh,nzh),y(nxh,nyh,nzh),z(nzh),zedge(nzh+1)  ! for metryc
...
c  determine the amount of canopy directly above a given x and y location
            rhocolumn=0.0
            if (itminxh.le.ix.and.itmaxxh.ge.ix.and.    ! trees and grass
     +          itminyh.le.iy.and.itmaxyh.ge.iy) then
              do ic=icminzh,icmaxzh
                ztopcell=zcart(zedge(ic+1),ix,iy,zs,zb)-zs(ix,iy)
                zbotcell=zcart(zedge(ic),ix,iy,zs,zb)-zs(ix,iy)
                rhocolumn=rhocolumn+
     +                 (rhof(ix,iy,ic)*(ztopcell-zbotcell))
              enddo
            else
              rhocolumn=0.0
            endif
...
c  of litter - determine if there is grass possible in the cell
            if (igminxh.le.ix.and.igmaxxh.ge.ix.and.    !identify areas with grass
     +          igminyh.le.iy.and.igmaxyh.ge.iy) then
...
c  computed rhocolumn
              shadefactor=exp(-1.0*grassconstant*
     +                    (rhocolumn/max(rhomax*avgcanht,rhocolumn)))
c
...
c  is full and the shading
              do iz=1,izg
                ...
                actualgroundload=actualrhograss ! fuel load @ ground if linear etrap
     +                   -.5*actualgrassht*fueldistributionslope 
                rhomicro=500.            ! kg/m3
                ztopcell=zcart(zedge(iz+1),ix,iy,zs,zb)-zs(ix,iy)
                zbotcell=zcart(zedge(iz),ix,iy,zs,zb)-zs(ix,iy)
                rhograss=shadefactor*(min(actualgrassht,ztopcell)-
     ...
     +                       /(rhograss+rhof(ix,iy,iz))
                rhof(ix,iy,iz)=rhof(ix,iy,iz)+rhograss
...
c  check to see if we are in a litter area 
            if (ilminxh.le.ix.and.ilmaxxh.ge.ix.and.    !identify areas with litter
     +          ilminyh.le.iy.and.ilmaxyh.ge.iy) then
c  izl locates the top cell of the litter at a particular x and y on the higrad grid
              izl=1
              zltopcell=zcart(zedge(izl+1),ix,iy,zs,zb)-zs(ix,iy)
              do while (zltopcell.lt.actuallitterht)
                izl=izl+1
                zltopcell=zcart(zedge(izl+1),ix,iy,zs,zb)-zs(ix,iy)
              enddo
c  coverfactor is the fraction of the grass that remains under the
c  computed rhocolumn
              coverfactor=1.-exp(-1.0*rlitterconstant*
     +                 (rhocolumn/max(rhomax*avgcanht,rhocolumn)))
c
...
c  is full and the shading
              do iz=1,izl
                actualfuelbottom= 0.     ! inserting 0 for ground
                fueldistributionslope=0. ! pos # -> load higher at top (kg/m3/m)
                actualgroundload=actualrholitter ! fuel load @ ground if linear etrap
     +                       -.5*actuallitterht*fueldistributionslope
                rhomicro=500.            ! kg/m3
                ztopcell=zcart(zedge(iz+1),ix,iy,zs,zb)-zs(ix,iy)
                zbotcell=zcart(zedge(iz),ix,iy,zs,zb)-zs(ix,iy)
                rholitter=coverfactor
     ...
     +                    *rholitter
     +                    +rhof(ix,iy,iz)*
     +                    min(actualfueldepth(ix,iy,iz),ztopcell)
     +                    -zbotcell)
     +                    /(rholitter+rhof(ix,iy,iz))
                rhof(ix,iy,iz)=rhof(ix,iy,iz)+rholitter
...
c     enddo
c
c  determine fuel moisture based on whether there are trees, grass,
c  litter, or neither
c     moist=0.00001
      do iz=1,nzh
        do iy=1,nyh
     ...
     +                       (rhofgrass(ix,iy,iz)*fmoistgr)+
     +                       (rhoflitter(ix,iy,iz)*fmoistlitter))/
     +                       (rhoftree+rhofgrass(ix,iy,iz)+
     +                                      rhoflitter(ix,iy,iz))
            endif
          enddo
        enddo
...
c
      ...
      subroutine read_angelfire
      use treedatinfo
c
c  read excel spreadsheet data (that has been converted to text)
c  that contains the data from Sandia National Laboratory collected
c  in Angelfire, NM
c
c     write (*,1000)
c1000 format(1x,'Tree data file (Excel converted to text): ',$)
c     read (*,*) treefile
c
      treefile='angelfire_unthinned.txt'
      open (1,file=treefile,form='formatted',status='old')
      read (1,'(a)') dummy
      read (1,'(a)') dummy
      ntreedat=1
      do while (ios.ge.0)
        read (1,1100,iostat=ios) itreenum(ntreedat),dbh(ntreedat),
     +       height(ntreedat),cr1(ntreedat),cr2(ntreedat),
     +       htlc(ntreedat),species(ntreedat)
 1100   format(i10,f6.1,f8.1,7x,f7.2,f7.2,f13.1,a5)
       print *,'rec ',ntreedat
       print *,'#',itreenum(ntreedat)
       print *,'DBH=',dbh(ntreedat)
       print *,'Height=',height(ntreedat)
       print *,'CrownRad 1=',cr1(ntreedat)
       print *,'CrownRad 2=',cr2(ntreedat)
       print *,'HTLC=',htlc(ntreedat)
       print *,'Species=',species(ntreedat)
        if (ios.lt.0) then
          ntreedat=ntreedat-1     ! end-of-file
        else
          ntreedat=ntreedat+1     ! not a blank line in excel file
      ...
      enddo
      print *,ntreedat,' trees in Sandia unthinned Angel Fire data'
      close (1)
c
c  NOTE:  cr1 and cr2 are computed as half the crown diameter which was 
c         measured in the Excel spreadsheets. Also, the measured data was
c         tree circumference at 4.5 ft, not dbh, so we are converting as 
c         best we can.
      do i=1,ntreedat
       height(i)=height(i)*.3048
       cr1(i)=cr1(i)*.3048
       cr2(i)=cr2(i)*.3048
       htlc(i)=htlc(i)*.3048
       dbh(i)=(dbh(i)/3.14159265)*.0254 
      enddo
...
c  leave the tree if it is right on the edge of the HIGRAD grid
     ...
     +                                   (northy*yslope*value))
c        if (it.eq.48) print *,'JLW48 ',-slopeeffectval,
c    +   northx,xslope,value,northy,yslope,value,nsslopefactor
            call random_number(randomnumber)
            retainpercent=(1.+nsslopefactor)/(1.0+slopeeffectval)
            if (randomnumber.ge.retainpercent) itreeh(it)=-999
c           if (it.le.100) print *,it,xloch(it),yloch(it),xslope,
c    +                  yslope,nsslopefactor,retainpercent,
c    +                  randomnumber,slopeeffectval,value,ix,iy,
c    +                  zs(ix+1,iy),zs(ix-1,iy),zs(ix,iy+1),
c    +                  zs(ix,iy-1)
          endif
...
c  temp arrays and variables for reading tree data
     ...
     +       htdctmp(ntr),htdttmp(ntr),cr1tmp(ntr),cr2tmp(ntr)
        character speciestmp(ntr)*8,livetmp(ntr)*1,spptmp(ntr)*3,
