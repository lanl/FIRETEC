...
subroutine advec(xv,il,iu,jl,ju,lls)
  ...
  call setAdvectiveVelocities(uavg,vavg,oavg,il,iu,jl,ju,lls,dt,iord) ! this is advvel in code. 
  call mpdata(xv,h,xe,il,iu,jl,ju,lls,nv)
...
subroutine setAdvectiveVelocities(u,v,o,il,iu,jl,ju,lls,dta,order)
  use gridlist_variables, only : l,ih,ibclatopen
  use forcings, only : u1,u2,u3
  use xvall, only : iuvel,ivvel,irho,xe,nv
  use metric_variables, only : gi
  ...
  real,intent(in) :: dta
  real,intent(inout) :: u(il:iu,jl:ju,lls),v(il:iu,jl:ju,lls)
  real,intent(inout) :: o(il:iu,jl:ju,lls)
...
!----------------------------------------------------------------
! mpdata what's a good description of this routine???
!----------------------------------------------------------------
subroutine mpdata(x,h,xe,il,iu,jl,ju,lls,nvp)
  use gridlist_variables, only : l,ih,iord,nonos,idiv,nfct,nonosold
  use forcings, only : u1,u2,u3
  use metric_variables, only : gi
  ...
  real,intent(in) :: h(il:iu,jl:ju,lls)
  real,intent(inout) :: x(il:iu,jl:ju,lls,nvp),xe(il:iu,jl:ju,lls,nvp)
  
  real,external :: donor,vcorr,vdiv1,vdiv2,vdyf,pn,pp
  ...
  real,allocatable:: v1mp(:,:,:,:),v2mp(:,:,:,:),v3mp(:,:,:,:)
  real,allocatable:: f1(:,:,:,:),f2(:,:,:,:),f3(:,:,:,:)
  real,allocatable:: f1o(:,:,:,:),f2o(:,:,:,:),f3o(:,:,:,:)
  real,allocatable:: cpmp(:,:,:,:),cnmp(:,:,:,:)
  real,allocatable:: mxo(:,:,:,:),mno(:,:,:,:),mx(:,:,:,:),mn(:,:,:,:)
  real,allocatable::  a(:,:,:,:)

  ! Executable Code
  n3=l+1
  ep=1.e-10
  allocate (v1mp(il:iu+1,jl:ju  ,lls  ,nvp))
  allocate (v2mp(il:iu  ,jl:ju+1,lls  ,nvp))
  allocate (v3mp(il:iu  ,jl:ju  ,lls+1,nvp))
  allocate (f1(il:iu+1,jl:ju  ,lls  ,nvp)  )
  allocate (f2(il:iu  ,jl:ju+1,lls  ,nvp)  )
  allocate (f3(il:iu  ,jl:ju  ,lls+1,nvp)  )
  allocate (f1o(il:iu+1,jl:ju  ,lls  ,nvp) )
  allocate (f2o(il:iu  ,jl:ju+1,lls  ,nvp) )
  allocate (f3o(il:iu  ,jl:ju  ,lls+1,nvp) )
  allocate (cpmp(il:iu,jl:ju,lls,nvp)      )
  allocate (cnmp(il:iu,jl:ju,lls,nvp)      )
  allocate (mx(il:iu,jl:ju,lls,nvp-1)      )
  allocate (mn(il:iu,jl:ju,lls,nvp-1)      )
  allocate (mxo(il:iu,jl:ju,lls,nvp)       )
  allocate (mno(il:iu,jl:ju,lls,nvp)       )
  allocate (a(il:iu,jl:ju,lls,nvp-1)       )
  ...
  do itr=1,iord ! loop to LINE 1402  
    do kv=1,nvp
      ...
      if (topdedge.eq.0) then                       
        ...
        call update(v2mp(1-ih,1-ih,1,kv),v2mp(1-ih,1-ih,1,kv),np,mp+1,l, &
                    1-ih,np+ih,1-ih,mp+ih+1,3,0) 
      endif
    ...
    if(itr.ne.iord)then  !! Skip if iord.eq.1: start of mpdata when iord=2: escaping to LINE 1403
      ...
      do kv=1,nvp
        ...
        do k=2,l
          do j=jllim,julim
            do i=illim,iulim
              v3mp(i,j,k,kv)=vdyf(x(i,j,k-1,kv),x(i,j,k,kv),f3(i,j,k,kv), &
                .5*(h(i,j,k-1)+h(i,j,k)))                                 &
                +vcorr(f3(i,j,k,kv),                                      &
        ...
        if (rightdedge.eq.0) then                       
          call update(v1mp(1-ih,1-ih,1,kv),v1mp(1-ih,1-ih,1,kv),np,mp,l, &
           1-ih,np+ih+1,1-ih,mp+ih,1,0)  
        else
          call update(v1mp(1-ih,1-ih,1,kv),v1mp(1-ih,1-ih,1,kv),np+1,mp,l, &
           1-ih,np+ih+1,1-ih,mp+ih,1,0)  
        endif
      ...
      if(nonosold.eq.1) then  !                       non-osscilatory option
        do kv=1,nvp
          do k=1,l
            ...
            do j=1,mp
              ...
              do i=1,np
                ...
                mno(i,j,k,kv)=amin1(x(im,j,k,kv),x(i,j,k,kv), &
          ...
          call update(cpmp(1-ih,1-ih,1,kv),cpmp(1-ih,1-ih,1,kv),np,mp,l, &
           1-ih,np+ih,1-ih,mp+ih,0,0) 
          call update(cnmp(1-ih,1-ih,1,kv),cnmp(1-ih,1-ih,1,kv),np,mp,l, &
           1-ih,np+ih,1-ih,mp+ih,0,0) 
        ...
        !!! correction coeffiecients for rho
            ...
            do k=1,l
              do j=1,mp
                do i=1,np
                  ...
                  cpmp(i,j,k,nvp)=amin1(tmpp,cpmp(i,j,k,nvp))
                  cnmp(i,j,k,nvp)=amin1(tmpn,cnmp(i,j,k,nvp))
                enddo !i=1:np
              enddo !j=1:mp
            enddo !k=1:l
          enddo !kv=1:nv-1 

          call update(cpmp(1-ih,1-ih,1,nvp),cpmp(1-ih,1-ih,1,nvp),np,mp,l, &
           1-ih,np+ih,1-ih,mp+ih,0,0) 
          call update(cnmp(1-ih,1-ih,1,nvp),cnmp(1-ih,1-ih,1,nvp),np,mp,l, &
           1-ih,np+ih,1-ih,mp+ih,0,0) 
          ...
          if(itrfct.lt.nfct) then
            do kv=1,nvp
              ...
              if(rightdedge.eq.0) then                       
                call update(f1o(1-ih,1-ih,1,kv),f1o(1-ih,1-ih,1,kv),np,mp,l, &
                 1-ih,np+ih+1,1-ih,mp+ih,2,0) 
              else
                call update(f1o(1-ih,1-ih,1,kv),f1o(1-ih,1-ih,1,kv),np+1,mp,l, &
                 1-ih,np+ih+1,1-ih,mp+ih,2,0) 
              endif
 
              if(botdedge.eq.0) then                       
                call update(f2o(1-ih,1-ih,1,kv),f2o(1-ih,1-ih,1,kv),np,mp,l, &
                 1-ih,np+ih,1-ih,mp+ih+1,3,0) 
              else
                call update(f2o(1-ih,1-ih,1,kv),f2o(1-ih,1-ih,1,kv),np,mp+1,l, &
                 1-ih,np+ih,1-ih,mp+ih+1,3,0) 
              endif
...
real function vdiv2(aa,b1,b2,b3,b4,r)
  Implicit None
  real,intent(in) :: aa,b1,b2,b3,b4,r
...
real function vcorr(aa,b,y0,y1,y2,y3,r)
  Implicit None
  real,intent(in) :: aa,b,y0,y1,y2,y3,r
  real,external :: rat4
