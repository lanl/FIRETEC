...
subroutine update(a,ae,nxp,nyp,nzp,il,iu,jl,ju,icorner,iperiod)
  use gridlist_variables, only : ih,nprocy,ibclatopen
  use msga_variables, only : mpi_real,ierror,mpi_status_size,mpi_real, &
    pebelow,peabove,peleft,peright,mpi_comm_world,botdedge,topdedge, &
    leftdedge,rightdedge,peleftbelow,perightbelow,peleftabove,perightabove
  use gridsetup, only : timestep
  Implicit None

  ! Local Variables
  integer,intent(in) :: nxp,nyp,nzp,il,iu,jl,ju,icorner,iperiod
  real,intent(in) :: ae(il:iu,jl:ju,nzp)
  real,intent(inout) :: a(il:iu,jl:ju,nzp)

  integer :: i,j,k
  integer :: i0,j0
  integer :: nxbuf,nybuf,ncorbuf
  integer :: icnt,itag,iref,jref,ncount
  real,allocatable :: tmpxsnd(:),tmpxrcv(:),tmpysnd(:),tmpyrcv(:), &
    tmpcorsnd(:),tmpcorrcv(:),sndrcvStatus(:)
  ...
  allocate(sndrcvStatus(mpi_status_size))
