...
subroutine fieldUpdate()
  use gridlist_variables, only : l,dx,dy,dz,ih,iturb
  use xvall, only : xv,xvrho,irho,ika,ikb,iuvel,ivvel,iwvel,nv
  use metric_variables, only : gmul,gi,c13,c23
  use gridsetup, only : np,mp
  use turb_variables, only : K_axy,K_az,K_b,sqrtG_Kxy,sqrtG_Kz, &
  ...
  do kv=1,nv
    call update(xvrho(:,:,1:l,kv),xvrho(:,:,1:l,kv),np,mp,l,1-ih,np+ih,1-ih,mp+ih,1,0)
  enddo
  call update(rtke_abc(:,:,1:l),rtke_abc(:,:,1:l),np,mp,l,1-ih,np+ih,1-ih,mp+ih,1,0)
  ...
  do j=1-ih+1,mp+ih-1
    do i=1-ih+1,np+ih-1
      ...
      do kv=1,nv
        xvrho(i,j,0,kv)=getSubGroundValue(xvrho(i,j,k,kv),xvrho(i-1,j,k,kv), &
          xvrho(i+1,j,k,kv),xvrho(i,j-1,k,kv),xvrho(i,j+1,k,kv),g13,g23,g33)
      enddo
