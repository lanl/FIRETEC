...
subroutine diffuse(fphi, phi,il,iu,jl,ju,lls,cdiff)
  use gridlist_variables, only : ih,l,dx,dy,dz,rturbprandtl
  use gridsetup, only : np,mp,dt,dxi,dyi,dzi
  use metric_variables, only : c13,c23,gmul,gi
  use turb_variables, only : sqrtG_Kxy,sqrtG_KG33
  use msga_variables, only : mpi_rank
  Implicit None

  ! Local Variables
  integer,intent(in) :: il,iu,jl,ju,lls
  real,intent(inout) :: fphi(il:iu, jl:ju,lls) ! rhs of variable to diffuse
  real,intent(in) :: phi(il:iu, jl:ju,0:lls) ! variable to diffuse (defined on 0:l)
  real,intent(in) :: cdiff  ! ratio between vt of variable iv and simple vt (ie rturbprandtl for theta and o2)
  ...
  !noise filter
  real :: phi_diff1, phi_diff2, phi_diff3, phi_diff4
  ...
  hdyi=0.5*dyi
  hdzi=0.5*dzi
  allocate(hxc(np+1,mp,l))
  allocate(hyc(np,mp+1,l))
  allocate(hzc(np,mp,l+1))
