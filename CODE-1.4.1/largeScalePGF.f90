...
subroutine rinitLargeScalePGF
  ...
  use msga_variables, only : mpi_rank,mpi_real,mpi_sum,ierror, &
    mpi_comm_world
  ...
  integer :: i,j,k
  real :: cosg,sing
  real :: intsintheta,intsinthetal,zla,gam,dzcell
  real :: z1,z2,z12,zcoef
  real :: targMassFluxSum
  real,external :: zcart
...
subroutine addToMassFluxTemp(i,j,k,isIni,zfactor)
  use gridlist_variables, only : l,dx,dy
  ...
  real,intent(in) ::zfactor  !generally a dz or a cell fraction...
  integer :: ia,ja
  integer :: j0f
  real :: j0,dj0f  ! index in massflux array
...
subroutine largeScalePGF
  use gridlist_variables, only : l,izlspgf,zu,frqlspgf,ilspgf, &
    dx,dy,n,m,nprocx,nprocy
  use constants, only : pi
  use gridsetup, only : np,mp,ittot,dt
  use metric_variables, only : z,zs,zedge
  use msga_variables, only : mpi_rank,mpi_real,mpi_sum, &
    mpi_comm_world,ierror,npos,mpos,numprocs
