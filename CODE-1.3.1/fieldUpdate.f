...
! - sqrtG_GK33 = sqrtG*(Kxy * J31**2 + Kxy * J32**2 +Kz * J33**2) ! generalised metric tensor term
     ...
     +           +sqrtG_Kz(i,j,k) * gi(i,j,k)**2
      ...
      call updated(u(:,:,1:l),u(:,:,1:l),np,mp,l,1-ih,np+ih,1-ih,mp+ih,1,0)
      call updated(v(:,:,1:l),v(:,:,1:l),np,mp,l,1-ih,np+ih,1-ih,mp+ih,1,0)
      call updated(w(:,:,1:l),w(:,:,1:l),np,mp,l,1-ih,np+ih,1-ih,mp+ih,1,0)
      call updated(theta(:,:,1:l),theta(:,:,1:l),np,mp,l,1-ih,np+ih,1-ih,mp+ih,1,0)
      call updated(tke_a(:,:,1:l),tke_a(:,:,1:l),np,mp,l,1-ih,np+ih,1-ih,mp+ih,1,0)
      call updated(tke_b(:,:,1:l),tke_b(:,:,1:l),np,mp,l,1-ih,np+ih,1-ih,mp+ih,1,0)
      call updated(ox(:,:,1:l),ox(:,:,1:l),np,mp,l,1-ih,np+ih,1-ih,mp+ih,1,0)
      call updated(rtke_abc(:,:,1:l),rtke_abc(:,:,1:l),np,mp,l,1-ih,np+ih,1-ih,mp+ih,1,0)
      ...
      call updated(sqrtG_Kxy,sqrtG_Kxy,np,mp,l,1-ih,np+ih,1-ih,mp+ih,0,0)
      call updated(sqrtG_Kz,sqrtG_Kz,np,mp,l,1-ih,np+ih,1-ih,mp+ih,0,0)
      call updated(sqrtG_KG33,sqrtG_KG33,np,mp,l,1-ih,np+ih,1-ih,mp+ih,0,0)
      
!      if (topofile(1:1).eq.'*'.or.topofile.eq.' ') then
!      do j=1-ih,mp+ih
!      do i=1-ih,np+ih
!         u(i,j,0) = u(i,j,1)
!         v(i,j,0) = v(i,j,1)
...
!         tke_a(i,j,0) = tke_a(i,j,1)
!         tke_b(i,j,0) = tke_b(i,j,1)
!         ox(i,j,0) = ox(i,j,1)
!         rtke_abc(i,j,0) = rtke_abc(i,j,1)
!      enddo
!      enddo


!      else ! topofile=> 3 ghost cells are required for bc
...
!      endif ! test on topofile
      ...
      if (irhovapor.eq.1) then
         ...
         enddo
         enddo
         call updated(vap(:,:,1:l),vap(:,:,1:l),np,mp,l,1-ih,np+ih,1-ih,mp+ih,1,0)
         k=1 !bottombc
!         if (topofile(1:1).eq.'*'.or.topofile.eq.' ') then
!             do j=1-ih,mp+ih
!             do i=1-ih,np+ih
