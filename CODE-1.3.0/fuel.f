      ...
      subroutine fuel(i,j,k,rneteng,ff,fw)
      use xve
      use xvo
      use metryic
      use fireteca
...
c scale.  The .2 factor should be replaced by a function of sc and sb.
      rkctemp=.2*xvfuel(i,j,k,5)/xvb(i,j,k,nv)
...
c computation of psif
       psif =computePsi (temps(i,j,k),ifuel)


c computation of thetasolid and cf
      ! if (rhofinitial(i,j,k).gt.0.0) then
         perchydroremaining=max(0.,
     .             (rhof(i,j,k)-rhohydrothresh*rhofinitial(i,j,k))
     .                 /(rhofinitial(i,j,k)*(1.-rhohydrothresh)))
         cf=cfhydro*perchydroremaining
     .          +cfchar*(1-perchydroremaining)
       !else
       !  cf(i,j,k)=0.0
       !endif
         hydrofactor=exp(-1.*rno/rnfuel*psif
     &                        *rhof(i,j,k)/xvb(i,j,k,7))
         thetasolid=perchydroremaining*(1.0-thetag)*hydrofactor
...
c      tapsw=tamb+80.
      th=2*twvap-tapsw
      if(temps(i,j,k).le.tapsw) then
          psiw=0.
      else if(temps(i,j,k).ge.th) then
          psiw=1.
      else
          psiw=(temps(i,j,k)-tapsw)/(th-tapsw)        !(rrl 6/1/99)
      endif
      psiwmaxold=psiwmax(i,j,k)
      psiwmax(i,j,k)=max(psiwmax(i,j,k),psiw)



c computation of ff
      slambdaof=rhof(i,j,k)*xvb(i,j,k,7)/(rhof(i,j,k)/rnfuel+ !(rrl 6/1/99)
     .                                xvb(i,j,k,7)/rno)**2
      ff= ffparam*cf*rhof(i,j,k)*xvb(i,j,k,7)*sigmac*
     .          psif*slambdaof/(rhoref*sx*sx)     !(rrl 6/1/99)
c computation of fw
      if (psiwmax(i,j,k).lt.1.and.rhowater(i,j,k).gt.0.0) then
          fw=min(rhowater(i,j,k)*(psiwmax(i,j,k)-psiwmaxold)/
...
c  update rhof, rhos, rhowater
        rhofold=rhof(i,j,k)
        rhowaterold=rhowater(i,j,k)
        rhosold=rhos(i,j,k)
        siesold=sies(i,j,k)
        ! TODO: tempsold is unused, to remove
...
c update sies, temps
     ...
     +                    /rhos(i,j,k)
        ...
        rneteng=ff*hf*(1.-thetasolid)  !for gas phase
