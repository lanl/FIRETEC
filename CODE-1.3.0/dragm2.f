	...
	! , increases for lighter canopy and decrease for more dense
        ! total area
         ...
         if (k.eq.1) then
             fuelcorrection=sqrt(actualfueldepth(i,j,k)/ztopcell)
             xvfuel(i,j,k,1)=xv(i,j,k,1)*fuelcorrection
             xvfuel(i,j,k,2)=xv(i,j,k,2)*fuelcorrection
             xvfuel(i,j,k,3)=xv(i,j,k,3)*fuelcorrection
             xvfuel(i,j,k,4)=xv(i,j,k,5)*fuelcorrection**2.0
             xvfuel(i,j,k,5)=xv(i,j,k,6)*fuelcorrection**2.0
	     dragxt=(.5*cd(i,j,k)*av+0.1)*sp
	     dragyt=(.5*cd(i,j,k)*av+0.1)*sp
             dragzt=.5*cd(i,j,k)*sp*av
         else
	     ...
	     dragxt=.5*cd(i,j,k)*sp*av
             dragyt=.5*cd(i,j,k)*sp*av
             dragzt=.5*cd(i,j,k)*sp*av
          endif
         !formerly implicit
